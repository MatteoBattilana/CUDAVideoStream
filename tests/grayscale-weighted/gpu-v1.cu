...
__global__ void grayscale(uint8_t *color, uint8_t *grayscale, int maxSect) {
    ...
    for (int i = start; i < max; i++) {
        grayscale[i] = 0.114 * color[i*3] + 0.587 * color[i*3 + 1] + 0.299 * color[i*3 + 2];
    }
}
...
int main(int argc, char const *argv[]) {
    ...
    while (1) {
        ...
        cudaMemcpyAsync(d_color, frame.data, total_color, cudaMemcpyHostToDevice);

        grayscale<<<1, nMaxThreads>>>(d_color, d_grayscale, maxAtTime);

        cudaMemcpyAsync(output.data, d_grayscale, total_grayscale, cudaMemcpyDeviceToHost);
